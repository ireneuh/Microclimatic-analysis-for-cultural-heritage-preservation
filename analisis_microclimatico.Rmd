...
datos_brutos_informe <- read_excel("datos_sensor.xls", 
...
db <- read_excel("datos_sensor.XLS", skip=11) #crear el objeto de datos brutos sobre el que trabajar
...
#### Tabla resumen:
`r kable(tabla_db, caption="Tabla Análisis Global Temperatura (ºC) y Humedad Relativa (%)",format="html",booktabs=T) %>%
kable_styling() %>%
...
OH_m$`dia-hora` <- as.POSIXlt(OH_m$dia, format="%d/%m/%Y %H")
